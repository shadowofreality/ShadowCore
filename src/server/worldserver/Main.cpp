...
extern int main(int argc, char** argv)
{
    ...
    if (!sConfigMgr->LoadInitial(configFile.generic_string(),
                                 ...
                                 configError))
    {
        printf("Erro no arquivo de configuração: %s\n", configError.c_str());
        return 1;
    }
    ...
    if (!pidFile.empty())
    {
        ...
        {
            TC_LOG_ERROR("server.worldserver", "Não é possivel criar PID file %s.\n", pidFile.c_str());
            return 1;
        }
    }
    ...
    if (networkThreads <= 0)
    {
        TC_LOG_ERROR("server.worldserver", "Network.Threads está maior que 0");
        return false;
    }
    ...
    if (int coreStuckTime = sConfigMgr->GetIntDefault("MaxCoreStuckTime", 0))
    {
        ...
        TC_LOG_INFO("server.worldserver", "Iniciando anti-freeze thread (%u seconds max stuck time)...", coreStuckTime);
    }

    TC_LOG_INFO("server.worldserver", "%s (worldserver-daemon) pronto...", GitRevision::GetFullVersion());
    ...
    TC_METRIC_EVENT("events", "Worldserver desligado", "");
    sMetric->ForceSend();
